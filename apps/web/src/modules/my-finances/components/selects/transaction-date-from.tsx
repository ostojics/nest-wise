...
interface PrivateTransactionDateFromPickerProps {
  className?: string;
}

const PrivateTransactionDateFromPicker: React.FC<PrivateTransactionDateFromPickerProps> = ({className}) => {
  ...
  const handleSelectDate = (value: Date | undefined) => {
    if (!value) return;
    void navigate({
      search: (prev) => ({...prev, transactionDate_from: formatSelectedDate(value), page: 1}),
      to: '/my-finances',
    });
    setOpen(false);
  };
...
export default PrivateTransactionDateFromPicker;
