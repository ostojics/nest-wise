...
// Confirm AI Transaction Suggestion
// Schema for confirming an AI suggestion and creating the transaction
export const confirmAiTransactionSuggestionHouseholdSchema = z
  .object({
    accountId: z
      .string({
        required_error: 'Račun je obavezan',
    ...
    isReconciled: z
      ...
      .default(true),
    // Fields from the AI suggestion for category creation
    newCategorySuggested: z.boolean(),
    suggestedCategoryName: z
      .string({
        invalid_type_error: 'Neispravna vrednost (mora biti tekst)',
      })
      .max(100, 'Ime kategorije može imati najviše 100 karaktera')
      .optional(),
  })
  .strict()
  .superRefine((val, ctx) => {
    ...
    // For expenses, require either categoryId OR newCategorySuggested with suggestedCategoryName
    if (val.type === 'expense') {
      const hasCategory = val.categoryId !== null;
      const hasNewCategorySuggestion = val.newCategorySuggested && val.suggestedCategoryName;

      if (!hasCategory && !hasNewCategorySuggestion) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          path: ['categoryId'],
          message: 'Rashodne transakcije moraju imati kategoriju',
...
export type ConfirmAiTransactionSuggestionHouseholdDTO = z.infer<typeof confirmAiTransactionSuggestionHouseholdSchema>;
